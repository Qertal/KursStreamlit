...
\begin{figure}[H] 
    \centering
    \includegraphics[width=0.8\textwidth]{figure/Screenshot_2.png}
...
\begin{enumerate}
    ...
    \item Tutaj wybieramy ile punktów będzie w naszej przestrzeni. Wybierając $>10$ punktów, nie zostanie wyświetlona macierz odległości, z racji na jej rozmiar.
...
Teraz z lewego panelu wybieramy przycisk \textbf{Odległość między zbiorami}, co przenosi nas do sekcji poświęconej obliczaniu odległości między dwoma zbiorami (\ref{rys:omz}). 
...
W tej sekcji aplikacji możemy rysować kule otwarte, domknięte oraz sfery w przestrzeni dwuwymiarowej. Aby skorzystać z tej funkcji, należy kliknąć przycisk \textbf{Kule na płaszczyźnie} (tak, sfera nie jest kulą), co przeniesie nas do sekcji (\ref{rys:rk}). 
...
Warto wspomnieć, że zostało wprowadzone tutaj trochę ograniczeń. Są one skutkiem ograniczeń przy rysowaniu wykresów i generowaniu punktów, zarówno ze strony logicznej jak i technicznej (ograniczenia złożoności obliczeniowej). Głównie chodzi o to, że:
\begin{enumerate}
...
Jak widać, aplikacja jest stosunkowo prosta zarówno w budowie, jak i w obsłudze. Mimo to spełnia swoje zadanie: umożliwia obliczanie średnicy zbiorów, odległości między zbiorami oraz – dodatkowo – rysowanie kul otwartych, domkniętych i sfer w przestrzeni dwuwymiarowej.

W ramach dalszego rozwoju aplikacji warto rozważyć dodanie obsługi wyrażeń niewymiernych przy obliczaniu średnicy i odległości między zbiorami. Można także wprowadzić możliwość wyboru pomiędzy metodami obliczeń numerycznych a symbolicznymi, co pozwoliłoby na uzyskanie dokładniejszych wyników.

Takie narzędzie może być szczególnie przydatne dla studentów matematyki znajdujących się na początku swojej drogi w poznawaniu zagadnień topologii.
